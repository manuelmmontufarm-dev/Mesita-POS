...
router.get(['/settings', '/settings/'], asyncHandler(async (req, res) => {
...
router.patch(['/settings', '/settings/'], asyncHandler(async (req, res) => {
  const restaurantId = req.auth?.restaurant?.id;
  if (!restaurantId) return res.status(401).json({ error: 'Unauthorized' });
...
router.post(['/setup', '/setup/'], asyncHandler(async (req, res) => {
  const restaurantId = req.auth?.restaurant?.id;
  if (!restaurantId) return res.status(401).json({ error: 'Unauthorized' });
